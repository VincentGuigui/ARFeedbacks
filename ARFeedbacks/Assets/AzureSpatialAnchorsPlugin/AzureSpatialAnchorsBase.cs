...
{
    public abstract class AzureSpatialAnchorsBase : InputInteractionBase
    {
        protected Func<CloudSpatialAnchor, GameObject> GameObjectSelector;
        ...
        protected void LogAndDisplay(string text)
        {
            this.feedbackBox.text = text;
            Debug.Log(text);
        }
...
//                objectToMove.transform.rotation = worldRot;
        ...
        protected virtual void OnSaveCloudAnchorFailed(Exception exception)
        {
            ...
            this.feedbackBox.text = string.Format("!");
        ...
        protected override void OnSelectObjectInteraction(Vector3 hitPoint, object target)
        {
            if (this.IsPlacingObject())
            {
                ...
                this.feedbackBox.text = "Placing the emotion...";
                this.SpawnOrMoveCurrentAnchoredObject(hitPoint, rotation);
            }
        }
        ...
        protected virtual void SaveCurrentObjectAnchorToCloud()
        {
            ...
            localCloudAnchor.AppProperties["name"] = this.spawnedObject.name;
        ...
        protected virtual GameObject SpawnNewAnchoredObject(Vector3 worldPos, Quaternion worldRot, GameObject anchoredObject)
        {
            GameObject newGameObject = GameObject.Instantiate(anchoredObject, worldPos, worldRot);
        ...
        /// <param name="cloudSpatialAnchor">The cloud spatial anchor.</param>
        /// <returns><see cref="GameObject"/>.</returns>
        protected virtual GameObject SpawnObjectFromAnchor(Vector3 worldPos, Quaternion worldRot, CloudSpatialAnchor cloudSpatialAnchor)
        {
            GameObject selectedObjectToInstantiate = ObjectToAttach;
            GameObject newGameObject;
            if (GameObjectSelector != null)
                selectedObjectToInstantiate = GameObjectSelector.Invoke(cloudSpatialAnchor);
            if (selectedObjectToInstantiate == null)
            {
                LogAndDisplay("No selected object to instantiate");
                return null;
            }
            else
                newGameObject = this.SpawnNewAnchoredObject(worldPos, worldRot, selectedObjectToInstantiate);
        ...
        protected virtual void SpawnOrMoveCurrentAnchoredObject(Vector3 worldPos, Quaternion worldRot)
        {
            ...
            if (this.spawnedObject == null)
            {
                LogAndDisplay("Create new emotion");
                this.spawnedObject = this.SpawnObjectFromAnchor(worldPos, worldRot, this.currentCloudAnchor);

                this.spawnedObjectMat = this.spawnedObject.GetComponent<MeshRenderer>().material;
            }
            else
            {
                LogAndDisplay("Move existing emotion");
        ...
        private void CloudManager_OnAnchorLocated(object sender, AnchorLocatedEventArgs args)
        {
            ...
            this.feedbackBox.color = Color.green;
            if (args.Status == LocateAnchorStatus.Located)
            {
                LogAndDisplay("OnAnchorLocated");
                this.feedbackBox.text = "OnAnchorLocated";
        ...
        private void CloudManager_OnLocateAnchorsCompleted(object sender, LocateAnchorsCompletedEventArgs args)
        {
            ...
            LogAndDisplay("CloudManager_OnLocateAnchorsCompleted");
        }

        private void CloudManager_SessionUpdated(object sender, SessionUpdatedEventArgs args)
        {
            this.OnCloudSessionUpdated();
            if (args.Status.ReadyForCreateProgress < 1f)
                this.feedbackBox.text = "Please scan a bit more:" + args.Status.UserFeedback.ToString() + " (" + args.Status.RecommendedForCreateProgress.ToString("#.0") + ")";
        }

        private void CloudManager_OnSessionError(object sender, SessionErrorEventArgs args)
        {
            this.isErrorActive = true;
            LogAndDisplay(string.Format("Can't load session: Error {0}", args.ErrorMessage));
        }
